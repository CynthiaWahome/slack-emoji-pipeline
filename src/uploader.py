...
EXCLUDE_ASSETS = {name.strip() for name in os.getenv("EXCLUDE_ASSETS", "").split(",") if name.strip()}
...
def _find_browser_executable() -> str:
    ...
    return ""  # Fallback to Playwright default
...
async def fetch_live_list(page: Page) -> Set[str]:
    ...
    try:
        ...
        response = await page.evaluate(f"fetch('/api/emoji.list?token={token}').then(r => r.json())")
        if response.get("ok"):
...
async def execute_upload():
    ...
    all_files = sorted([f for f in NAMED_DIR.iterdir() if f.is_file() and not f.name.startswith(".")])
    ...
    async with async_playwright() as pw:
        ...
        for p in context.pages[1:]: await p.close()
        ...
        for i, filepath in enumerate(targets):
            ...
            if name in live_list:
                logger.info("[%d/%d] Skipping :%s: (Live)", i+1, len(targets), name)
                skip_count += 1
                continue
            ...
            try:
                ...
                await page.locator("input[type='file']").first.set_input_files(str(filepath.resolve()))
                await page.locator("[data-qa='customize_emoji_name_input']").first.fill(name)
                
                # Check for Save readiness
